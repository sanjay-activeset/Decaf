...
let headingsmob = document.querySelectorAll("[heromob=word]");
let textLinesmob = document.querySelectorAll("[heromob='line']");

ScrollTrigger.matchMedia({
  // Apply animation for mobile, tablets, and landscape mode
  "(max-width: 1024px)": function () {
    headingsmob.forEach((heading) => {
      heading.style.overflow = "hidden";
      let splitText = new SplitType(heading, { types: "words" });
    ...
    textLinesmob.forEach((el) => {
      el.style.overflow = "hidden";
      let text = new SplitType(el, { types: "lines" });
      ...
      gsap.to(text.lines, {
        ...
        ease: "power3.out",
        scrollTrigger: {
          trigger: el, // Each text line triggers its own animation
          start: "top 80%",
          toggleActions: "play none none none",
          once: true,
        },
